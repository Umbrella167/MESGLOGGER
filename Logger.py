...
class Logger:
    def __init__(self, output_dir="logs"):
        ...
        self.message_index = 0  # 初始化消息索引

        # 创建日志文件名，包含日期和时间
        log_file_name = (
            now.strftime("Rec_%Y-%m-%d_%H-%M-%S-") + f"{now.microsecond}.log"
        )
    ...
    def log(
        self,
        message_data,
        message_type=log_pb2.MessageType.MESSAGE_PROTO,
        save_module="RealTime",
        size=0,
        energy_saving=False,
    ):
        """
        ...
        with self.lock:
            ...
            if energy_saving and self.previous_log_message is not None:
                if (
                    log_message.message_data == self.previous_log_message.message_data
                    and log_message.message_type
                    == self.previous_log_message.message_type
                ):
                    return
